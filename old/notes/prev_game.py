...
class GuiGameClient:
    ...
    def render(self):
        ...
        # Players box title
        title = self.font.render(f"Players in {self.location or 'Unknown'}", True, (200, 200, 200))
        ...
        if self.state_data:
            ...
            item_height = 40
            start_y = MARGIN + TOP_INFO_HEIGHT + 50  # Start below title
            ...
            for i, (pid, data) in enumerate(players_in_room.items()):
                ...
                # Player text
                player_text = f"Player {pid[:8]}"  # Show only first 8 chars of ID
                if pid == self.player_id:
                    player_text += " (You)"
        ...
        if self.available_exits:
            ...
            button_margin = 10
            total_buttons_width = (button_width + button_margin) * len(self.available_exits)
